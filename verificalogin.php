...
elseif (isset($_COOKIE["loginautomatico"])){
 $log = utf8_encode(htmlspecialchars($_COOKIE["loginadp"]));
...
try
{
    ...
    if (count($resultados) != 1) {
        ...
        setcookie("loginadp", $log, time()+60*60*24*90); //guarda o login por 90 dias
           if(!empty($lembrar)) {
           setcookie("loginautomatico", $senha, time()+60*60*24*90); //guarda a senha por 90 dias
               
        }
           $_SESSION['auth']=true;
		   $_SESSION['nomeCompleto'] = $resultados[0]['nomeFunc'];
		   $_SESSION['nomeUsuario'] = $log;
		   header("Location:principal.php"); //chama a pagina principal
		   die();    
